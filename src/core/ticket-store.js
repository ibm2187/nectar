...
class TicketStore extends EventEmitter {
  ...
  /**
   * Compute a forward-looking delivery risk forecast for a release.
   *
   * Answers "will this release ship on time?" by comparing actual velocity
   * (tickets completed in the last 14 days) to the required velocity
   * (remaining tickets / days until deadline).
   *
   * @param {string} version - Release version string
   * @param {object} [opts]
   * @param {number} [opts.velocityWindowDays=14] - Days to look back for velocity
   * @param {number} [opts.bufferDays=2] - Days before release date to use as deadline
   * @param {Date}   [opts.now] - Override current date (for testing)
   * @returns {object} Forecast object with risk assessment
   */
  getDeliveryForecast(version, opts = {}) {
    const velocityWindowDays = opts.velocityWindowDays || 14;
    const bufferDays = opts.bufferDays != null ? opts.bufferDays : 2;
    const now = opts.now || new Date();
    const todayStr = now.toISOString().slice(0, 10);

    // 1. Get all tickets for this version
    const tickets = this.getForVersion(version);
    const total = tickets.length;

    // 2. Classify tickets into status groups
    ...
    for (const t of tickets) {
      ...
      } else if (BLOCKED_SET.has(t.status)) {
        breakdown.blocked++;
      } else if (QA_READY_SET.has(t.status)) {
        // Distinguish "Cherry Picked" / "Retest After Cherrypick" as awaitingCp
        const cpStatuses = ['Cherry Picked', 'Cherrypick is Building', 'Retest After Cherrypick'];
        if (cpStatuses.includes(t.status)) {
          breakdown.awaitingCp++;
        } else {
          breakdown.readyForQa++;
        }
      } else if (IN_QA_SET.has(t.status)) {
        breakdown.inQa++;
      } else if (IN_DEV_SET.has(t.status)) {
        // Classify "To Do", "Backlog", "Open", etc. as notStarted vs in-dev
        const notStartedStatuses = ['Open', 'To Do', 'Backlog', 'Planning', 'Requirements',
          'Needs Requirements', 'Ready to Develop', 'Ready For Estimation', 'Pending'];
        if (notStartedStatuses.includes(t.status)) {
          breakdown.notStarted++;
        } else {
          breakdown.inDev++;
        }
      } else {
        // Unknown status — treat as in-dev
        breakdown.inDev++;
      }
    }
    ...
    // 3. Get release date from releases table
    const releaseRow = this.db.prepare(
      'SELECT jiraReleaseDate, createdAt FROM releases WHERE version = ?'
    ).get(version);
    const releaseDate = releaseRow?.jiraReleaseDate || null;
    const releaseCreatedAt = releaseRow?.createdAt || null;

    // 4. Calculate deadline (release date minus buffer)
    let deadlineDate = null;
    let daysLeft = null;
    if (releaseDate) {
      const rd = new Date(releaseDate + 'T00:00:00');
      rd.setDate(rd.getDate() - bufferDays);
      deadlineDate = rd.toISOString().slice(0, 10);

      // Days left = calendar days from today to deadline
      const todayMs = new Date(todayStr + 'T00:00:00').getTime();
      const deadlineMs = rd.getTime();
      daysLeft = Math.round((deadlineMs - todayMs) / (1000 * 60 * 60 * 24));
    }

    // 5. Calculate velocity: tickets completed in the velocity window for this version
    const windowStart = new Date(now);
    windowStart.setDate(windowStart.getDate() - velocityWindowDays);
    const windowStartStr = windowStart.toISOString().slice(0, 10);

    const pattern = `%"${version}"%`;
    let completedInWindow = 0;
    let velocitySource = 'version';

    // Primary: count Done tickets updated within the window
    const velocityRow = this.db.prepare(`
      SELECT COUNT(*) AS n FROM jira_tickets
      WHERE (fixVersions LIKE ? OR targetFixVersions LIKE ?)
        AND statusCategory = 'Done'
        AND updatedInJira >= ?
    `).get(pattern, pattern, windowStartStr);
    completedInWindow = velocityRow?.n || 0;

    // Fallback 1: if updatedInJira is mostly empty, use release age
    if (completedInWindow === 0 && done > 0 && releaseCreatedAt) {
      const createdMs = new Date(releaseCreatedAt).getTime();
      const nowMs = now.getTime();
      const releaseAgeDays = Math.max(1, Math.round((nowMs - createdMs) / (1000 * 60 * 60 * 24)));
      completedInWindow = done;
      // Scale: actual velocity per day = done / releaseAgeDays, then * window
      const effectiveWindow = Math.min(releaseAgeDays, velocityWindowDays);
      completedInWindow = Math.round((done / releaseAgeDays) * effectiveWindow);
      velocitySource = 'release-age';
    }

    // Fallback 2: global velocity across all active releases
    if (completedInWindow === 0) {
      const globalRow = this.db.prepare(`
        ...
        WHERE statusCategory = 'Done'
          ...
          AND fixVersions != '[]'
      `).get(windowStartStr);
      completedInWindow = globalRow?.n || 0;
      velocitySource = completedInWindow > 0 ? 'global' : 'none';
    }

    const actualVelocity = completedInWindow / velocityWindowDays; // tickets per day

    // 6. Required velocity
    let requiredVelocity = null;
    if (daysLeft !== null && daysLeft > 0) {
      requiredVelocity = remaining / daysLeft;
    } else if (daysLeft !== null && daysLeft <= 0) {
      requiredVelocity = remaining > 0 ? Infinity : 0;
    }
    ...
    if (remaining === 0) {
      risk = 'low';
      riskMessage = 'All tickets done';
    } else if (daysLeft !== null && daysLeft <= 0) {
      risk = 'critical';
      riskMessage = `Overdue -- ${remaining} tickets remaining, deadline was ${deadlineDate}`;
    } else if (requiredVelocity !== null && actualVelocity > 0) {
      if (requiredVelocity <= actualVelocity * 0.8) {
        risk = 'low';
        riskMessage = `On track -- need ${requiredVelocity.toFixed(1)}/day, team doing ${actualVelocity.toFixed(1)}/day`;
      } else if (requiredVelocity <= actualVelocity * 1.5) {
        risk = 'medium';
        riskMessage = `At risk -- need ${requiredVelocity.toFixed(1)}/day, team doing ${actualVelocity.toFixed(1)}/day`;
      } else {
        risk = 'high';
        riskMessage = `Behind -- need ${requiredVelocity.toFixed(1)}/day, team doing ${actualVelocity.toFixed(1)}/day`;
      }
    } else if (actualVelocity === 0 && remaining > 0) {
      risk = daysLeft !== null ? 'high' : 'unknown';
      riskMessage = remaining > 0 && daysLeft !== null
        ? `No velocity data -- ${remaining} tickets remaining, ${daysLeft} days left`
        : `No velocity data -- ${remaining} tickets remaining`;
    }

    // 8. Projected completion date
    let projectedDate = null;
    if (actualVelocity > 0 && remaining > 0) {
      const daysNeeded = remaining / actualVelocity;
    ...
    return {
      ...
      velocity: {
        ...
        required: requiredVelocity !== null ? Math.round(requiredVelocity * 100) / 100 : null,
