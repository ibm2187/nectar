...
const KNOWN_COLUMNS = [
  ...
  'releaseType','shipDate','milestones','templateVersion','shippedAt',
  'createdAt','updatedAt',
];
...
class ReleaseManager extends EventEmitter {
  ...
  transition(version, toState, user = null) {
    ...
    // Set shippedAt when entering 'done' — immutable record of ship time
    // (used by on-time ship trend reporting; do not derive from updatedAt)
    if (toState === 'done' && !release.shippedAt) {
      release.shippedAt = release.updatedAt;
    }
